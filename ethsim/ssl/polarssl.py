...
class RSA:
    ...
    def encrypt_public(self, data):
        ...
        """
        data = "\x00" * (self._keylen - len(data)) + data
        input = ctypes.create_string_buffer(data, self._keylen)
        output = ctypes.create_string_buffer("\x00", self._keylen)
        ret = lib.rsa_public(self._ctx, input, output)
        return output.raw
    ...
    def encrypt_private(self, data):
        ...
        input = ctypes.create_string_buffer(data, self._keylen)
        output = ctypes.create_string_buffer("\x00", self._keylen)
        ret = lib.rsa_private(self._ctx, input, output)
        return output.raw
