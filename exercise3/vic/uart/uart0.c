...
volatile uint8_t uart0TxBufferAvailable = 1;
volatile bool uart0RxBufferReadyFlag = false;
...
void __attribute__ ((interrupt("IRQ"))) handleUART() {
  if(uart0TxInProgress&&(U0LSR & BIT5)) { //We are in the middle of transmission
    sendNextByte();
    if(!bytesLeftToTx())
      endTransfer();
  }
  ...
  VICSoftIntClr = 1<<INT_SOURCE_UART0;
  vicUpdatePriority();
}
  ...
  if(bytesLeftToTx())
    {
      ...
      U0IER |= BIT1;
      //Used to workaround slow-start initialization, described on page 10 of the manual
      VICSoftInt = 1<<INT_SOURCE_UART0;
    }
  else
    endTransfer();
}
...
static void uart0Init() {
  ...
  vicInit();
  vicEnableIRQ(INT_SOURCE_UART0, 0, handleUART);
  U0IER = BIT0;
