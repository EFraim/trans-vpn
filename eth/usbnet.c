...
bool usb_control_class_handler(void) {
  uint32_t oid;
  uint32_t buflen;
  uint32_t bufptr;
...
void usb_cdc_ecm_rx(uint8_t ep, uint8_t stat) {
    ...
    if (recv_ring.size == 0 || recv_ring_drop) {
        ...
        int len = MIN(buffer->length - buffer->current, MAX_PACKET_SIZE);
...
void usb_cdc_ecm_tx(uint8_t ep, uint8_t stat) {
    ...
    len = MIN(buffer->length - buffer->current, MAX_PACKET_SIZE);
    ...
    if (buffer->length == buffer->current && len < MAX_PACKET_SIZE) {
